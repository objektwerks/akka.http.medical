...
class Store(conf: Config) {
  ...
  def findDietNutritionByPatientEncounterId(patiendId: Long, encounterId: Long): Future[Option[DietNutrition]] = DB readOnly { implicit session =>
    val result = sql"""select PERSON_ID as Patient,
